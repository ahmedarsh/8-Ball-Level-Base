...
public class ShotPowerScript : MonoBehaviour {
    ...
    void Start() {
		...
		instance = this.gameObject.GetComponent<ShotPowerScript> ();
    ...
    void OnMouseUp() {
        ...
        if (!GameManager.Instance.stopTimer && cueScript.isServer) {
            ...
            if (cueScript.steps > 0) {
				...
				GlobalGameHandler.isShoot = true;

				m_GameControllerScript.mGameController.pausebtn.Play ("pausebtnAnim");
				m_GameControllerScript.mGameController.accuracy.Play ("accuracyAnim");
				m_GameControllerScript.mGameController.ballpots.Play ("ballpotOut");
				m_GameControllerScript.mGameController.ballHide.SetActive (false);
            }

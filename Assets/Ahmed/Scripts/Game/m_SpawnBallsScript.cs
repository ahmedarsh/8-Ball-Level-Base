...
public class m_SpawnBallsScript : MonoBehaviour {
    ...
    void Start() {
        ...
        //16
		balls = new GameObject[m_GameControllerScript.mGameController.Positions.Length+1];
		renderers = new MeshRenderer[m_GameControllerScript.mGameController.Positions.Length+1];
		rigidbodies = new Rigidbody[m_GameControllerScript.mGameController.Positions.Length+1];
		enabledScripts = new LockZPosition[m_GameControllerScript.mGameController.Positions.Length+1];
		...
		for (int i=0;i<m_GameControllerScript.mGameController.Positions.Length;i++) {

			GameObject ball = Instantiate(ballPrefab);


			ball.transform.tag = "Ball";
			print(balls.Length);
			print("position length  "+m_GameControllerScript.mGameController.Positions.Length);
			balls[i+1] = ball;
			renderers[i+1] = ball.GetComponent<MeshRenderer>();
			rigidbodies[i+1] = ball.GetComponent<Rigidbody>();
			enabledScripts[i+1] = ball.GetComponent<LockZPosition>();
...
//			print ("i        I    "+i);
			ballRigidbody.transform.position=m_GameControllerScript.mGameController.Positions[i].transform.position;
			ballRigidbody.transform.rotation=m_GameControllerScript.mGameController.Positions[i].transform.rotation;
			ballRigidbody.transform.name = m_GameControllerScript.mGameController.Positions [i].transform.name;
			ball.GetComponent<Renderer>().material.SetTexture("_MainTex", texturesArray[i]);
...
//					ballRigidbody.transform.position=GameControllerScript.gameController.Positions[counter].position;
